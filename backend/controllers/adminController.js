...
const deleteClient = async (req, res) => {
  try {
    ...
    if (!password) {
      return res
        .status(400)
        .json({ message: "Admin password is required to delete a client" });
    }
...
const resetClientPassword = async (req, res) => {
  try {
    ...
    client.password = newPassword; // pre-save hook will hash it
    await client.save();

    res.json({ message: `Password reset successfully for ${client.userName}` });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
